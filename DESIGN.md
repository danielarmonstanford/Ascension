...
**The Sensory Chapter Rule.** Every sensory lockup follows one order: dominant Lovine chapter hero → copper utility line naming the modalities → evocative Lovine sub-hero at approximately half scale. This applies to EMBODY, SEE, SOUND, TASTE, BREATHE, CREATE and the nested MOVE experience, where ECSTATIC DANCE sits immediately beneath MOVE. INTUITION belongs in CREATE's copper utility line, not in its hero. On desktop the copper line may offset toward the opposing edge, but it must remain below its hero; on mobile it may wrap but never move above the hero. Preserve this sequence independently across desktop, tablet and mobile.
...
**The TASTE Film Rule.** The approved culinary film replaces only the existing TASTE media placeholder. Keep the chapter’s asymmetric type composition and copy unchanged, use the film’s own frame as the fallback, remain muted, and crop the film to fill the established media rectangle without letterboxing.

**The BREATHE Stage Rule.** The approved Cloudinary respiratory film is the full-bleed BREATHE chapter ground, not a contained media card. Layer the interface independently above it, screen the motion for legibility, and render BREATHE in pale Pacific blue so it remains the dominant hero. Keep the film muted, inline and progressive, retain its first-frame poster as the static and reduced-motion fallback, and do not imply a medical diagnosis, treatment or outcome.

**The SOUND Stage Rule.** SOUND has two deliberate responsive compositions built from the same progressive media architecture. Desktop is a full-bleed Vietnamese film stage: screened Night Sea motion behind oversized Lovine, copper modalities, support copy and the two listening controls. Mobile begins with the ear illustration, then progressively crossfades into the same muted, inline Vietnamese film when the chapter enters view and playback is ready. MEDITATE NOW opens a contained Tibetan sound player over the moving stage; FILM SOUND explicitly enables or mutes the film audio, and film audio and meditation remain mutually exclusive. Reduced-motion and failed playback retain the ear illustration. Never collapse the desktop treatment back into a video card on a pale section.
...
**The Commercial Scan Rule.** On sponsorship and partner offer pages, decision-critical numbers are primary reading content, not utility labels. Sponsorship investment amounts use large, high-contrast Lovine numerals with currency and tier context separated into a smaller but fully legible utility line. A visitor must be able to scan package name → investment → fit → deliverables in that order on desktop and mobile without relying on colour alone.
